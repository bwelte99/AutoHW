  ...
  with HasBoomFrontendParameters
{
  ...
  when (RegNext(do_commit_update || do_repair_update || do_mispredict_update)) {
    ...
    first_empty := false.B
    if(BRANCH_PRINTF){
      when (io.bpdupdate.bits.cfi_is_br && io.bpdupdate.valid ){
        printf("bu: %x %x %x %x\n", io.bpdupdate.bits.cfi_taken, io.bpdupdate.bits.pc, io.bpdupdate.bits.target, io.bpdupdate.bits.cfi_mispredicted)
      }
    }
  }
