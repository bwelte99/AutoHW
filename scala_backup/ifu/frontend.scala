  ...
  with HasBoomFrontendParameters
{
  ...
  val debug_branch_address = RegInit(0.U(32.W))
  ...
  if (BRANCH_PRINTF && false) {
    for (b <- 0 until nBanks) {
      for (w <- 0 until bankWidth) {
        val i = (b * bankWidth) + w  
        when(f3_fetch_bundle.insts(i) === 49772643.U){
         debug_branch_address := bankAlign(f3_fetch_bundle.pc)
        }
      }
    }

    for( i <- 0 until fetchWidth){
      when( s1_vpc === debug_branch_address ){
          printf("f1: %x %x %x\n", s1_bpd_resp.preds(i).taken, s1_vpc, s1_bpd_resp.preds(i).predicted_pc.valid)
      }
      when( s2_vpc === debug_branch_address ){
          printf("f2: %x %x %x\n", f2_bpd_resp.preds(i).taken, s2_vpc, f2_bpd_resp.preds(i).predicted_pc.valid)
      }
      when( f3_aligned_pc === debug_branch_address ){
          printf("f3: %x %x %x\n", f3_bpd_resp.io.deq.bits.preds(i).taken, s1_vpc, f3.io.deq.valid)
      }
    }
  }
