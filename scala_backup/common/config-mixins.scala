...
class WithNBoomBranchReportPrintf(n: Int) extends Config((site, here, up) => {
  case TilesLocated(InSubsystem) => up(TilesLocated(InSubsystem), site) map {
    case tp: BoomTileAttachParams => tp.copy(tileParams = tp.tileParams.copy(core = tp.tileParams.core.copy(
      enableBranchReportPrintf = true 
    )))
    case other => other
  }
})
...
class WithNCustomBooms(n: Int = 1, overrideIdOffset: Option[Int] = None) extends Config(
  new WithCustomTAGELBPD ++ // Default to TAGE-L BPD
  new Config((site, here, up) => {
    case TilesLocated(InSubsystem) => {
      val prev = up(TilesLocated(InSubsystem), site)
      val idOffset = overrideIdOffset.getOrElse(prev.size)
        ...
        BoomTileAttachParams(
          tileParams = BoomTileParams(
            core = BoomCoreParams(
              ...
              issueParams = Seq(
                IssueParams(issueWidth=MEM_ISSUE_WIDTH, numEntries=MEM_NUM_ENTRIES, iqType=IQT_MEM.litValue, dispatchWidth=MEM_DISPATCH_WIDTH),
                IssueParams(issueWidth=ALU_ISSUE_WIDTH, numEntries=ALU_NUM_ENTRIES, iqType=IQT_INT.litValue, dispatchWidth=ALU_DISPATCH_WIDTH),
                IssueParams(issueWidth=FPU_ISSUE_WIDTH, numEntries=FPU_NUM_ENTRIES, iqType=IQT_FP.litValue , dispatchWidth=FPU_DISPATCH_WIDTH)),
              numIntPhysRegisters = NUM_PHYS_INT_REG,
              numFpPhysRegisters = NUM_PHYS_FP_REG,
              numLdqEntries = NUM_LOAD_QUEUE,
              numStqEntries = NUM_STORE_QUEUE,
              maxBrCount = MAX_BRANCH_COUNT,
              numFetchBufferEntries = NUM_FETCH_BUFFER_ENTRIES,
              ftq = FtqParameters(nEntries=16),
              nPerfCounters = NUM_PERF_COUNTER,
              enablePrefetching = PRE_FETCHING,
              fpu = Some(freechips.rocketchip.tile.FPUParams(sfmaLatency=4, dfmaLatency=4, divSqrt=true))
            ),
            dcache = Some(
              DCacheParams(rowBits = site(SystemBusKey).beatBits, nSets=64, nWays=CACHE_WAYS, nMSHRs=NUM_MSHR, nTLBWays=TLB_WAYS)
            ),
            icache = Some(
              ICacheParams(rowBits = site(SystemBusKey).beatBits, nSets=64, nWays=CACHE_WAYS, fetchBytes=FETCH_BYTES)
            ),
            hartId = i + idOffset
          ),
          crossingParams = RocketCrossingParams()
        ...
        BoomTileAttachParams(
          tileParams = BoomTileParams(
            core = BoomCoreParams(
              ...
              // DO NOT CHANGE BELOW
              enableBranchPrintf = false, // I changed this :P (BW)
