  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  ...
  //initialize debug register to track when config is printed (BW)
  val config_print_ctr = RegInit(0.U) //width inferred to 1, which is ok
  
  //Print config info on 1st cycle (BW)
  if(CONFIG_PRINTF){
    when(config_print_ctr === 0.U){
      config_print_ctr := 1.U
      printf("**************************************************\n")
      printf("Attempting to print config to json ...\n")
      //print config to file
      val pw = new PrintWriter(new File("/root/volume/fuzzing_modified/chipyard/sims/verilator/fakeConfig.json"))
      //val pw = new PrintWriter(new File("$SIM_DIR/fakeConfig.json"))
      
      pw.print("{\n")         //file start
      
      pw.print("\t\"ls_queue_entries\": ")  //LS queue entries
      pw.print(issueParams(0).numEntries)
      pw.print(",\n")

      pw.print("\t\"int_queue_entries\": ") //Int queue entries
      pw.print(issueParams(1).numEntries)
      pw.print(",\n")

      pw.print("\t\"fp_queue_entries\": ")  //FP queue entries
      pw.print(issueParams(2).numEntries)
      pw.print(",\n")

      //TODO: FIX THESE; THEY ARE HARD CODED FOR NOW (2/2/23 - BW)
      pw.print("\t\"slowest_mem_lat\": " + 10 + ",\n") //(total guess)
      pw.print("\t\"slowest_int_lat\": " + 5 + ",\n") //(total guess)
      pw.print("\t\"slowest_fp_lat\": " + 27 + ",\n")

      pw.print("\t\"instr_wb_time\": " + 1 + "\n")
      
      pw.println("}")         //end of file
      pw.close
      
      printf("Done printing - you can cancel the simulation now :]\n");
      printf("**************************************************\n")
    }
  }
  ...
  // Value for finding load command
  val debug_load_address = RegInit(0.U(32.W))
...
//   "CSRFile is being written to spuriously.")
  ...
  if (COMMIT_LOG_PRINTF) {
    ...
    when(brupdate.b2.mispredict && !RegNext(rob.io.flush.valid) && b2.uop.debug_inst === 49772643.U ){
      printf("m: %x %x\n", b2.uop.debug_pc, debug_tsc_reg);
    }

    var new_ghist = debug_ghist

    for (w <- 0 until coreWidth) {
      when ( rob.io.enq_uops(w).debug_pc === debug_load_address && rob.io.enq_valids(w) ) {
        printf("le: %x %x %x\n",
                Sext(rob.io.commit.uops(w).debug_pc(vaddrBits-1,0), xLen),
      ...
      when ( rob.io.enq_uops(w).debug_inst === 49772643.U && rob.io.enq_valids(w) ) {
        printf("be: %x %x %x %x\n",
                rob.io.enq_uops(w).taken,
                Sext(rob.io.enq_uops(w).debug_pc(vaddrBits-1,0), xLen),
                rob.io.enq_uops(w).debug_inst,
                debug_tsc_reg);
      }

      when (rob.io.commit.uops(w).debug_pc === debug_load_address && rob.io.commit.valids(w)) {
        printf("ll: %x %x %x\n",
                Sext(rob.io.commit.uops(w).debug_pc(vaddrBits-1,0), xLen),
                rob.io.commit.uops(w).debug_inst,
                debug_tsc_reg);
      ...
      when (rob.io.commit.uops(w).debug_inst === 49772643.U && rob.io.commit.valids(w)) {
        debug_load_address := rob.io.commit.uops(w).debug_pc + 40.U 
       //when (rob.io.commit.arch_valids(w) &&
