...
//  Dump ROB contents for debugging (BW)
  debug_entry := DontCare // override in statements below

  val window_open = "hf8b1c".U
  val window_shut = "hf8b2b".U

  when((io.debug_tsc >= (window_open - "h30".U)) && (io.debug_tsc <= (window_shut + "h30".U))){
    when((io.debug_tsc >= window_open) && (io.debug_tsc <= window_shut)){
      printf("\nROB: Cycle %x (SSW OPEN!!!!!!!)\n", io.debug_tsc)
      printf("Current Head idx = %d Current Tail idx = %d\n", rob_head_idx, rob_tail_idx)
    }
    .otherwise{ 
      printf("\nROB: Cycle %x\n", io.debug_tsc)
      printf("Current Head idx = %d Current Tail idx = %d\n", rob_head_idx, rob_tail_idx)
    ...
    for (i <- 0 until numRobEntries){
      when(debug_entry(i).valid === 1.U){
        printf("Rob Entry: " + i + " PC: %x, Instruction: %x, Valid: %d, Busy: %d, Unsafe: %d, Exception: %d\n", debug_entry(i).uop.debug_pc, debug_entry(i).uop.debug_inst, debug_entry(i).valid, debug_entry(i).busy, debug_entry(i).unsafe, debug_entry(i).exception)
      }
    }
    printf("\n");
  }
  ...
  for (w <- 0 until coreWidth) {
    ...
    for(i <- 0 until numRobRows){
      debug_entry(w*(numRobRows)+i).valid     := rob_val(i)
      debug_entry(w*(numRobRows)+i).busy      := rob_bsy(i)
      debug_entry(w*(numRobRows)+i).unsafe    := rob_unsafe(i)
      debug_entry(w*(numRobRows)+i).uop       := rob_uop(i)
      debug_entry(w*(numRobRows)+i).exception := rob_exception(i)
    }
//  println(debug_entry)
//  println(rob_debug_inst_wdata)

    rob_debug_inst_wmask(w) := io.enq_valids(w)
    rob_debug_inst_wdata(w) := io.enq_uops(w).debug_inst

    when (io.enq_uops(w).debug_inst === 49772643.U && io.enq_valids(w)) {
      debug_load_address := io.enq_uops(w).debug_pc + 40.U 
    }
    ...
    for (i <- 0 until numWakeupPorts) {
      ...
      when (wb_resp.valid && MatchBank(GetBankIdx(wb_uop.rob_idx))) {
        if(BRANCH_PRINTF){
          when(rob_uop(row_idx).debug_inst === 49772643.U){
            printf("bb: %x %x %x\n",
                Sext(rob_uop(row_idx).debug_pc(vaddrBits-1,0), xLen),
                rob_uop(row_idx).debug_inst,
                io.debug_tsc);
          }
          when(rob_uop(row_idx).debug_pc === debug_load_address){
            printf("lb: %x %x %x\n",
                Sext(rob_uop(row_idx).debug_pc(vaddrBits-1,0), xLen),
                rob_uop(row_idx).debug_inst,
                io.debug_tsc);
